...
acme_standalone(){
    ...
    bash ~/.acme.sh/acme.sh --install-cert -d ${domain} --key-file /etc/acme/private.key --fullchain-file /etc/acme/cert.crt --ecc
    checktls
}
...
checktls() {
    if [[ -f /etc/acme/cert.crt && -f /etc/acme/private.key ]]; then
        if [[ -s /etc/acme/cert.crt && -s /etc/acme/private.key ]]; then
            if [[ -n $(type -P wg-quick) && -n $(type -P wgcf) ]]; then
                wg-quick up wgcf >/dev/null 2>&1
            fi
            if [[ -n $(type -P wg-quick) && -n $(type -P wgcf) ]]; then
            ...
            yellow "证书crt文件路径如下: /etc/acme/cert.crt"
            yellow "私钥key文件路径如下: /etc/acme/private.key"
            back2menu
        else
            if [[ -n $(type -P wg-quick) && -n $(type -P wgcf) ]]; then
                wg-quick up wgcf >/dev/null 2>&1
...
revoke_cert() {
    ...
    if [[ -n $(bash ~/.acme.sh/acme.sh --list | grep $domain) ]]; then
        ...
        rm -f /etc/acme/cert.crt /etc/acme/private.key
